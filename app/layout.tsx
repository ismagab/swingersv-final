...
export default async function RootLayout({ children }: Readonly<{ children: React.ReactNode }>) {
  const eventItems = await getAllEventsForSchema()
  const schemaEventsWithItems = { ...schemaEvents, itemListElement: eventItems }
    ...
    <html lang="es" className={`${_inter.variable} ${_playfair.variable}`}>
      <head>
        ...
        <Script id="schema-events" type="application/ld+json"
          dangerouslySetInnerHTML={{ __html: JSON.stringify(schemaEventsWithItems) }}
          strategy="beforeInteractive" />
      </head>
