...
export function ContactSection() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const whatsappEsposo = String(formData.get("whatsappEsposo") || "").trim()
    const whatsappEsposa = String(formData.get("whatsappEsposa") || "").trim()

    if (whatsappEsposo && whatsappEsposa && whatsappEsposo === whatsappEsposa) {
      setError(
        "El WhatsApp del esposo y de la esposa no pueden ser el mismo número. Por favor verifiquen los datos."
      )
      return
    }
    ...
    const textoMensaje =
      ...
      `*Esposo:* ${payload.nombreEsposo} (${payload.edadEsposo} años)%0A` +
      `*Esposa:* ${payload.nombreEsposa} (${payload.edadEsposa} años)%0A` +
      `*WhatsApp Esposo:* ${payload.whatsappEsposo}%0A` +
      `*WhatsApp Esposa:* ${payload.whatsappEsposa}%0A` +
      `*Departamento:* ${payload.departamento}%0A` +
            ...
            <div className="mb-8">
              ...
              <div className="grid gap-6 sm:grid-cols-2">
                <div className="flex flex-col gap-2">
                  <label htmlFor="nombreEsposo" className={labelClasses}>Nombre completo — Esposo</label>
                  <input id="nombreEsposo" name="nombreEsposo" type="text" required placeholder="Nombre completo" className={inputClasses} />
                </div>
                <div className="flex flex-col gap-2">
                  <label htmlFor="nombreEsposa" className={labelClasses}>Nombre completo — Esposa</label>
                  <input id="nombreEsposa" name="nombreEsposa" type="text" required placeholder="Nombre completo" className={inputClasses} />
                </div>
                <div className="flex flex-col gap-2">
                  <label htmlFor="whatsappEsposo" className={labelClasses}>WhatsApp — Esposo</label>
                  <input id="whatsappEsposo" name="whatsappEsposo" type="tel" required placeholder="+503 0000-0000" className={inputClasses} />
                </div>
                <div className="flex flex-col gap-2">
                  <label htmlFor="whatsappEsposa" className={labelClasses}>WhatsApp — Esposa</label>
                  <input id="whatsappEsposa" name="whatsappEsposa" type="tel" required placeholder="+503 0000-0000" className={inputClasses} />
                </div>
                <div className="flex flex-col gap-2">
                  <label htmlFor="edadEsposo" className={labelClasses}>Edad — Esposo</label>
                  <input id="edadEsposo" name="edadEsposo" type="number" min={18} required placeholder="18+" className={inputClasses} />
                </div>
                <div className="flex flex-col gap-2">
                  <label htmlFor="edadEsposa" className={labelClasses}>Edad — Esposa</label>
                  <input id="edadEsposa" name="edadEsposa" type="number" min={18} required placeholder="18+" className={inputClasses} />
                </div>
                <div className="flex flex-col gap-2 sm:col-span-2">
                  <label htmlFor="departamento" className={labelClasses}>Departamento de residencia</label>
                  <select
                    id="departamento"
                    name="departamento"
                    required
